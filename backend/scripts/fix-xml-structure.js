...
console.log('🔧 Construind cu structură XML CORECTĂ...');
...
try {
  ...
  // Verifică structura exactă din original pentru <w:rFonts> și <w:sz>
  // Sunt self-closing sau au closing tags?
  const sampleR = documentXml.match(/<w:rPr>.*?<w:rFonts.*?<\/w:rPr>/s);
  if (sampleR) {
    console.log('📋 Structură rPr găsită:');
    console.log(sampleR[0].substring(0, 200));
  }
  ...
  const addPlaceholder = (rowXml, placeholder) => {
    ...
    if (pattern.test(rowXml)) {
      // Folosește structura EXACTĂ din original - <w:rFonts> și <w:sz> sunt self-closing sau au closing tags?
      // Din original văd că sunt self-closing: <w:rFonts w:ascii="Times New Roman"/>
  ...
  // 3. Stampila - fără stampila pentru moment, doar placeholder-urile
  // Vom adăuga stampila separat dacă documentul funcționează
  console.log('\n📋 Pas 3: Stampila...');
  ...
  // Adaugă stampila în text box - structură simplificată
  const textBoxPattern = /(<wps:txbx>.*?<w:txbxContent>.*?<w:p[^>]*>.*?<w:t>Fdo\.<\/w:t>.*?<\/w:r>.*?<\/w:p>)(<\/w:txbxContent>)/s;
  
  if (textBoxPattern.test(documentXml)) {
    // Structură COMPACTĂ - toate tag-urile pe o linie pentru a evita probleme
  ...
  console.log('⚠️ Dacă documentul este corupt, problema este la stampila. Încearcă să deschizi documentul și spune-mi dacă funcționează.');
