/**
 * Backfill Nominas LONGBLOB (archivo) → Cloudflare R2.
 *
 ...
 *
 * Usage (from backend/):
 *   node scripts/nominas-r2-backfill.js .env.decamino.local --dry-run
 *   node scripts/nominas-r2-backfill.js .env.decamino.local --limit=50
 *   node scripts/nominas-r2-backfill.js .env.decamino.local --batch=20
 *   node scripts/nominas-r2-backfill.js .env.decamino.local --keep-blob
 *   npm run storage:nominas-backfill
...
function safeFileName(originalName, fallback = 'nomina') {
...
function buildNominaKey(tenant, codigoEmpleado, originalName, at) {
  const d = at instanceof Date && !Number.isNaN(at.getTime()) ? at : new Date();
  const yyyy = String(d.getUTCFullYear());
  const mm = pad2(d.getUTCMonth() + 1);
  const id = randomUUID();
  let name = originalName || 'nomina.pdf';
  if (!String(name).toLowerCase().endsWith('.pdf')) name = `${name}.pdf`;
  const safe = safeFileName(name);
  const scope = String(codigoEmpleado || '').trim() || 'sin-codigo';
  return [
    'decamino',
    tenant,
    'nominas',
...
async function main() {
  ...
  console.log('[nominas-backfill] env file:', envRel);
  console.log('[nominas-backfill] dry-run:', flags.dryRun);
  console.log('[nominas-backfill] keep-blob:', flags.keepBlob);
  console.log('[nominas-backfill] batch:', flags.batch);
  console.log('[nominas-backfill] limit:', flags.limit ?? 'none');
  ...
  if (!enabled) {
    console.error('[nominas-backfill] FAIL: R2_ENABLED is not true in', envRel);
    process.exit(1);
  }
  if (!accessKeyId || !secretAccessKey || !endpoint || !bucket) {
    console.error(
      '[nominas-backfill] FAIL: missing R2 credentials / endpoint / bucket',
    );
    process.exit(1);
  }
  ...
  if (!dbConfig.host || !dbConfig.user || !dbConfig.database) {
    console.error('[nominas-backfill] FAIL: missing DB_HOST / DB_USERNAME / DB_NAME');
    process.exit(1);
  }

  const tenant = tenantSlug(dbConfig.database);
  console.log('[nominas-backfill] database:', dbConfig.database);
  console.log('[nominas-backfill] tenant:', tenant);
  console.log('[nominas-backfill] bucket:', bucket);
  ...
  console.log('[nominas-backfill] pending rows:', pending);

  if (pending === 0) {
    await conn.end();
    console.log('[nominas-backfill] nothing to do');
    return;
  }
  ...
  while (processed < maxTotal) {
    ...
    const [rows] = await conn.query(
      `SELECT \`id\`, \`nombre\`, \`codigo_empleado\`, \`fecha_subida\`, \`archivo\`, \`tipo_mime\`
       FROM \`Nominas\`
       WHERE \`archivo\` IS NOT NULL
         AND (\`storage_key\` IS NULL OR \`storage_key\` = '')
       ORDER BY \`id\` ASC
    ...
    for (const row of rows) {
      ...
      try {
        ...
        const key = buildNominaKey(
          tenant,
          row.codigo_empleado,
          row.nombre || `nomina-${id}.pdf`,
          at,
        );
        ...
        await s3.send(
          new PutObjectCommand({
            ...
            Key: key,
            Body: buf,
            ContentType: row.tipo_mime || 'application/pdf',
            Metadata: {
              module: 'nominas',
              backfill: '1',
              nomina_id: String(id),
              codigo: String(row.codigo_empleado || ''),
            },
          }),
        );
        ...
        if (migrated % 10 === 0 || migrated === 1) {
          console.log(
            `[nominas-backfill] ok id=${id} (${migrated} migrated, ${failed} failed)`,
          );
        }
      } catch (err) {
        failed += 1;
        console.error(
          `[nominas-backfill] FAIL id=${id}:`,
          err?.message || err,
        );
      }
    }
  ...
  await conn.end();
  console.log('[nominas-backfill] done');
  console.log('[nominas-backfill] migrated:', migrated);
  console.log('[nominas-backfill] failed:', failed);
  console.log('[nominas-backfill] processed:', processed);
  if (failed > 0) process.exitCode = 1;
}

main().catch((err) => {
  console.error('[nominas-backfill] fatal:', err?.message || err);
  process.exit(1);
});
