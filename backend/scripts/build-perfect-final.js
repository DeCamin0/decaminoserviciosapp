...
console.log('🔧 Construind documentul PERFECT de la zero...');
...
try {
  ...
  // 1. Placeholder-uri (fără să modific structura)
  ...
  // 3. Stampila - FOARTE ATENT la structura text box-ului
  console.log('\n📋 Pas 3: Stampila...');
  ...
  // Găsește EXACT paragraful cu "Fdo." în text box și adaugă stampila DUPĂ el
  // Pattern: găsește paragraful care se termină cu "Fdo." și este în text box
  // Trebuie să fie ÎN <w:txbxContent> și ÎNAINTE de </w:txbxContent>
  const fdoInTextBox = /(<wps:txbx>.*?<w:txbxContent>.*?<w:p[^>]*>.*?<w:t>Fdo\.<\/w:t>.*?<\/w:r>.*?<\/w:p>)(<\/w:txbxContent>)/s;
  
  if (fdoInTextBox.test(documentXml)) {
    // Adaugă stampila ca paragraf NOU în text box, după "Fdo."
    // Structură simplă și corectă
    const stampilaPara = `
    ...
    documentXml = documentXml.replace(fdoInTextBox, `$1${stampilaPara}$2`);
    console.log('  ✅ Stampila adăugată în text box');
  } else {
    console.log('  ⚠️ Pattern nu a funcționat, încercând alternativ...');
  ...
  if (Math.abs(openTags - originalOpen) > 30 || Math.abs(closeTags - originalClose) > 30) {
    console.log('  ⚠️ Diferență mare - posibil corupt!');
  } else {
    console.log('  ✅ Structura pare OK');
  }
