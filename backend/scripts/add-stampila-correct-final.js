...
console.log('🔧 Construind documentul FINAL cu stampila CORECTĂ...');
...
try {
  ...
  console.log('  ✅ Toate placeholder-urile adăugate');
  ...
  // 3. Stampila - găsește exact unde este "Fdo." și adaugă stampila DUPĂ el
  console.log('\n📋 Pas 3: Stampila...');
  ...
  // Găsește paragraful cu "Fdo." - poate fi în text box sau nu
  // Pattern: găsește paragraful care se termină cu "Fdo." și adaugă stampila după el
  const fdoParaPattern = /(<w:p[^>]*>.*?<w:t>Fdo\.<\/w:t>.*?<\/w:r>.*?<\/w:p>)/s;
  
  if (fdoParaPattern.test(documentXml)) {
    // Adaugă stampila ca paragraf NOU după "Fdo."
    ...
    documentXml = documentXml.replace(fdoParaPattern, `$1${stampilaPara}`);
    console.log('  ✅ Stampila adăugată după "Fdo."');
  } else {
    console.log('  ⚠️ "Fdo." nu a fost găsit');
  }
