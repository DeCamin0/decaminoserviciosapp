...
import { IntentClassifierService, IntentType } from './intent-classifier.service';
...
export class AssistantService {
  ...
  async processMessage(messageDto: MessageDto): Promise<AssistantResponseDto> {
    ...
    try {
      ...
      let { intent, confianza, entidades } = intentResult;

      // 2. Verifică dacă e o întrebare de follow-up și completează entitățile din context
      const isFollowUp = this.contextService.isFollowUpQuestion(usuario.id, intent, entidades);
      if (isFollowUp) {
        this.logger.log(`🔗 Detectat follow-up pentru ${usuario.id}, completăm entitățile din context`);
        entidades = this.contextService.enrichEntitiesWithContext(usuario.id, entidades, intent) || entidades;
      ...
      if (intent === IntentType.DESCONOCIDO || confianza < 0.3) {
        this.logger.log(`🤖 Using AI for ${intent} intent (confianza: ${confianza.toFixed(2)})`);
      ...
      if (intent === IntentType.FICHAJES && !entidades?.fecha && !entidades?.mes) {
        ...
        if (!tieneReferenciaTemporal) {
          ...
          const clarificationResponse = await this.aiResponseService.generateClarificationRequest(
            intent,
            mensaje,
            usuario.rol,
          );
      ...
      try {
        switch (intent) {
          case IntentType.FICHAJES:
            ...
            this.logger.log(`🔍 [Assistant] FICHAJES intent detected. entidades.tipo: ${entidades?.tipo}`);
            if (entidades?.tipo === 'fichajes_faltantes') {
              this.logger.log(`🔍 [Assistant] Querying fichajes faltantes for fecha: ${entidades?.fecha || 'today'}`);
              data = await this.dataQuery.queryFichajesFaltantes(
                usuario.id,
                usuario.rol,
                entidades?.fecha,
              );
              this.logger.log(`✅ [Assistant] queryFichajesFaltantes returned ${data?.length || 0} results`);
            } else {
          ...
          case IntentType.VACACIONES:
            ...
            if (entidades?.mes) {
              ...
              const vacacionesData = await this.vacacionesService.calcularSaldo(usuario.id);
              data = {
                dias_anuales: vacacionesData.vacaciones.dias_anuales,
                dias_generados_hasta_hoy: vacacionesData.vacaciones.dias_generados_hasta_hoy,
                dias_consumidos_aprobados: vacacionesData.vacaciones.dias_consumidos_aprobados,
          ...
          case IntentType.EMPLEADOS:
            ...
            const mensajeLower = mensaje.toLowerCase();
            const tieneCuadranteHorario = mensajeLower.includes('cuadrante') || mensajeLower.includes('horario');
            const tieneCentro = mensajeLower.includes('centro') || mensajeLower.includes('centro de trabajo');
            
            if (tieneCuadranteHorario && tieneCentro && !entidades?.filtro) {
              // Mesaj cu două întrebări: executăm query-uri separate
              this.logger.log(`🔍 Detectat mesaj cu multiple întrebări: cuadrante/horario + centro`);
              
              const dataCuadranteHorario = await this.dataQuery.queryListadoEmpleados(
                usuario.id,
                usuario.rol,
              ...
              const codigosExistentes = new Set(dataCuadranteHorario.map((e: any) => e.CODIGO));
              ...
              data = combinedData;
              this.logger.log(`✅ Combinat ${dataCuadranteHorario.length} empleados sin cuadrante/horario + ${dataCentro.length} sin centro = ${data.length} total`);
            } else {
          ...
          case IntentType.PROCEDIMIENTOS:
            data = await this.dataQuery.queryKbArticles(
              undefined,
              mensaje,
            );
            break;
        ...
        this.logger.error(`❌ Error en query para intent ${intent}: ${error.message}`);
        data = null;
      }
      ...
      if (!data || (Array.isArray(data) && data.length === 0)) {
        ...
        if (confianza < 0.5 || queryError) {
          ...
          return {
            respuesta: 'No encuentro datos para esa consulta. He creado una incidencia para administración.',
      ...
      if (data && ((Array.isArray(data) && data.length > 0) || (typeof data === 'object' && Object.keys(data).length > 0))) {
        ...
        this.logger.log(`🤖 Using AI to format response with data for ${intent} intent`);
        ...
        const structuredResponse = await this.responseGenerator.generateResponse(
          intent,
        ...
        this.logger.log(`📊 Verificare acțiuni: data.length=${Array.isArray(data) ? data.length : 'N/A'}, acciones existente=${acciones.length}`);
        if (Array.isArray(data) && data.length > 10) {
          this.logger.log(`✅ Adăugare acțiuni de descărcare pentru ${data.length} înregistrări`);
          ...
          this.logger.log(`⚠️ Nu se adaugă acțiuni: data nu e array sau length <= 10`);
        }
        ...
        this.logger.log(`📤 Response ready: ${response.respuesta.substring(0, 100)}...`);
        this.logger.log(`📤 Response acciones: ${response.acciones?.length || 0} acțiuni`);
      } else {
        // Fără date - folosim AI pentru a răspunde natural
        this.logger.log(`🤖 Using AI to generate response without data for ${intent} intent`);
      ...
      return {
        respuesta: 'Ha ocurrido un error procesando tu consulta. Se ha creado una incidencia de alta prioridad.',
        confianza: 0.0,
        escalado: true,
        ticket_id: ticketId,
      };
