...
function RecentInspections() {
  ...
  const baseIntervalMs = 180000; // 3 minute în loc de 30s pentru a reduce rafalele
  const maxBackoffFactor = 4; // până la 12 minute
  const cacheRef = useRef({ data: null, timestamp: 0 });
  const cacheTtlMs = 4 * 60 * 1000; // 4 minute TTL pentru lista de inspecții

  const loadInspections = useCallback(async (force = false) => {
    const now = Date.now();
    if (!force && cacheRef.current.data && now - cacheRef.current.timestamp < cacheTtlMs) {
      setInspections(cacheRef.current.data);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const finalData = validInspections.slice(-5).reverse();
        setInspections(finalData);
        cacheRef.current = { data: finalData, timestamp: Date.now() };
        backoffRef.current = 1; // reset backoff pe succes
      } else {
        ...
        backoffRef.current =
          response.status >= 429
            ? maxBackoffFactor
            : Math.min(backoffRef.current * 2, maxBackoffFactor);
      }
    } catch (error) {
      console.error('Error loading inspections:', error);
      setInspections([]);
      backoffRef.current = Math.min(backoffRef.current * 2, maxBackoffFactor);
    } finally {
      setLoading(false);
      inFlightRef.current = false;
    }
  }, [maxBackoffFactor, cacheTtlMs]);
  ...
  useEffect(() => {
    ...
    const scheduleNext = (delayMs) => {
      ...
      pollTimeoutRef.current = setTimeout(runOnce, delayMs);
    };

    const runOnce = async () => {
      if (cancelled) return;
      if (document.hidden) {
        // când tab-ul e inactiv, reîncearcă mai târziu fără a lovi endpoint-ul
        scheduleNext(baseIntervalMs);
        return;
      }

      await loadInspections();
      const delay = baseIntervalMs * (backoffRef.current || 1);
      scheduleNext(delay);
    };
  ...
  }, [loadInspections, baseIntervalMs]);
