...
export default function SolicitudesPage() {
  ...
  const cacheTtlMs = 4 * 60 * 1000; // 4 minute pentru liste (inspecții/solicitudes/ausencias)
  ...
  // Polling control pentru solicitări/ausencias (reduce rafalele)
  const pollingTimeoutRef = useRef(null);
  const pollingBackoffRef = useRef(1);
  const pollingInFlightRef = useRef(false);
  const basePollingMs = 180000; // 3 minute (în loc de 60s)
  const maxPollingBackoffFactor = 4; // până la 12 minute
  ...
  useEffect(() => {
    ...
    const runOnce = async () => {
      if (cancelled) return;
      if (document.hidden) {
        // nu pollează când tab-ul e inactiv
