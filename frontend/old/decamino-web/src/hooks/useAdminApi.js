...
export const useAdminApi = () => {
  ...
  // Cache pentru permisiuni (TTL 5 minute)
  const permissionsCacheRef = useRef({});
  const permissionsTtlMs = 5 * 60 * 1000;
  const makePermKey = useCallback((grupo, module) => {
    if (grupo && module) return `${grupo}::${module}`;
    if (grupo) return `${grupo}::all`;
    return 'all';
  }, []);
  ...
  const getPermissions = useCallback(async (userGrupo = null, module = null) => {
    try {
      const cacheKey = makePermKey(userGrupo, module);
      const cached = permissionsCacheRef.current[cacheKey];
      if (cached && Date.now() - cached.timestamp < permissionsTtlMs) {
        return cached.data;
      }
      ...
      if (data && Array.isArray(data)) {
        ...
        permissionsCacheRef.current[cacheKey] = { data: permissions, timestamp: Date.now() };
        return permissions;
        ...
        permissionsCacheRef.current[cacheKey] = { data: defaults, timestamp: Date.now() };
        return defaults;
      }
    } catch (error) {
  ...
  const getAllPermissions = async () => {
    try {
      const cacheKey = makePermKey('admin', 'all');
      const cached = permissionsCacheRef.current[cacheKey];
      if (cached && Date.now() - cached.timestamp < permissionsTtlMs) {
        return cached.data;
      }
