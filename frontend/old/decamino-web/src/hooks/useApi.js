  ...
  const callApi = useCallback(async (url, options = {}) => {
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        // Unele webhook-uri răspund 200 cu body gol; evităm eroarea de parse
        ...
        if (!textBody || !textBody.trim()) {
          ...
          try {
            ...
            console.error('useApi JSON parsing error (application/json):', parseErr);
            throw new Error('Răspuns JSON nevalid de la server');
          }
        }
