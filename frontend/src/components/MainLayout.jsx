...
const getLogoUrl = () => {
  ...
  const isClient2Logo = (logoPath || '').toLowerCase().includes('hera');
  if (window.location.hostname.includes('ngrok') && !isClient2Logo) {
...
const MainLayout = ({ children }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-gray-50 dark:bg-gray-900 transition-colors" style={{
      ...
      {/* Fundal elegant – gradient neutru; decorațiuni cu culoarea brand (--primary-color per client) */}
      <div className="absolute inset-0 bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:from-gray-800 dark:via-gray-900 dark:to-gray-800">
        <div className="absolute inset-0 opacity-[0.07]" style={{ backgroundImage: 'radial-gradient(circle at 30px 30px, var(--primary-color, #E53935) 1px, transparent 0)', backgroundSize: '60px 60px' }} />
        {/* Elemente decorative – brand color cu opacitate */}
      ...
      <header className="sticky top-0 z-50 bg-white/80 dark:bg-gray-800/80 backdrop-blur border-b border-gray-200 dark:border-gray-700 relative">
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 flex items-center justify-between" style={{
          WebkitAlignItems: 'center',
          msFlexAlign: 'center',
          alignItems: 'center',
          ...
          <div className="flex items-center">
            <div className="flex-shrink-0">
              <div className="flex items-center">
                {/* Logo: HERA = integrat fără contur (ca pe login); DeCamino = cerc cu border primary */}
                <div className="relative mr-3">
                  {isClient2Logo() ? (
                    <img src={getLogoUrl()} alt={`${config.APP_NAME || config.COMPANY_NAME || 'App'} Logo`} className="h-14 w-auto max-w-[260px] object-contain drop-shadow-[0_2px_8px_rgba(0,0,0,0.12)]" onError={(e) => { e.target.style.display = 'none'; }} />
                  ) : (
                    <>
                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-lg border-4 border-primary-500">
                      <img src={getLogoUrl()} alt={`${config.APP_NAME || config.COMPANY_NAME || 'App'} Logo`} className="h-8 w-8 object-contain" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'block'; }} />
        ...
        <div className="md:hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3" style={{
            ...
            <div className="flex items-center flex-1 min-w-0">
              {/* Logo mic pentru mobile: HERA = integrat fără contur; DeCamino = cerc */}
              <div className="relative mr-2 flex-shrink-0">
                {isClient2Logo() ? (
                  <img src={getLogoUrl()} alt={`${config.APP_NAME || config.COMPANY_NAME || 'App'} Logo`} className="h-11 w-auto max-w-[180px] object-contain drop-shadow-[0_1px_6px_rgba(0,0,0,0.1)]" onError={(e) => { e.target.style.display = 'none'; }} />
                ) : (
                <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-lg border-2 border-primary-500">
                  <img src={getLogoUrl()} alt={`${config.APP_NAME || config.COMPANY_NAME || 'App'} Logo`} className="h-5 w-5 object-contain" onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'block'; }} />
              ...
              {/* Nume firmă din config (multi-client) */}
